...
int main(int argc, char* argv[]) {
    ...
    for (size_t i=0; i<max_iterations; i++) {
        ...
        case TransformationChoice::truncated:
            ...
            ///buf.resize_and_keep_capacity(buf.size() - rand.uintLessThan<size_t>(buf.size()));
            break;
        case TransformationChoice::insert_character:
            ...
            buf[tmp] = randAlphabeticAscii(rand);
            buf_size++;
            //buf.insert(rand.uintLessThan<size_t>(buf.size()), randAlphabeticAscii(rand));
            break;
        case TransformationChoice::append_character:
            buf[buf_size] = randAlphabeticAscii(rand);
            buf_size++;
            // buf.append(randAlphabeticAscii(rand));
            break;
        ...
        Web::HTML::NamedCharacterReferenceMatcher matcher;
        matcher.m_use_old_implementation = use_old;
